...
function random() {
    ...
    for (var j = 0; j < nAFC; j++) {
        if (j == rand)
            playModulatedNoise((j * carDur) + j * (ISI / 1000), carAmp, carDur, modAmp, modFreq, modPhase, onRamp, offRamp);
        else
    ...
    source.onended = () => { //quando l'oscillatore sta suonando il programma non si ferma, quindi serve questo per riattivare i pulsanti solo quando finisce
        for (var j = 1; j <= nAFC; j++) document.getElementById("button" + j).disabled = false;
    }
}
...
function select(button) {
    ...
    if (countRev == reversals + secondReversals) {
        ...
        location.href = "php/saveData.php?result=" + result + "&timestamp=" + timestamp + "&type=nmod" + description + "&currentBlock=" + currentBlock + "&score=" + score + "&saveSettings=" + saveSettings;
    }
