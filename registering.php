<?php		//apro la sessione per comunicare con le altre pagine del sito
    ...
    else{		//prendo tutti i dati		$psw = $_POST['psw'];		$name = $_POST['name'];		$surname = $_POST['surname'];		$date = $_POST['date'];		$gender = $_POST['gender'];		$notes = $_POST['notes'];				//creo il guest		$sql = "INSERT INTO guest (name, surname, date, gender, notes) VALUES ('$name', '$surname', '$date', '$gender', '$notes')";		$conn->query($sql);				//trovo il suo id		$sql = "SELECT MAX(id) as maxId FROM guest WHERE name='$name' AND surname = $surname AND date='$date' AND gender='$gender' AND notes='$notes'";		$result=$conn->query($sql);		$row=$result->fetch_assoc();		$id = $row['maxId'];				//creo e collego l'account, salvo l'hash della password con sha2-256
